...
class WizardSaft(models.Model):
    ...
    _description = ("Wizard exportador ficheiro Saft-T. Menu: Contabilidade -"
                    " Relatorios - Saft - Exportar Ficheiro Saft")
    ...
    def get_date_inicio(self):
        ...
        return str(datetime.date(day=1, month=last_month.month, year=last_month.year))
    ...
    def get_address(self, parent_tag, partner):
        ...
        for el, txt in zip(('AddressDetail', 'City', 'PostalCode', 'Country'),
    ...
    def act_getfile(self, state='get'):
        import datetime
        ...
        et.SubElement(header, 'AuditFileVersion').text = self.versao
        ...
        for element, texto in zip(('CompanyID', 'TaxRegistrationNumber', 'TaxAccountingBasis', 'CompanyName'),
                                  (vat, vat, self.tipo, partner_company.name[:60])):
        ...
        xml_txt = pretty_xml.encode('utf-8')
        out = base64.encodebytes(xml_txt)

        with open('/home/odoo/src/user/saft_agt_live_v17.xml',
                  'wb') as saft_file:
            saft_file.write(xml_txt)
            saft_file.close()

        self.write({'state': state, 'filedata': out, 'name': nome_file})
    ...
    def _write_products(self, master):
        ...
        for fatura in list_faturas:
            for linha_fatura in fatura.invoice_line_ids:
                if linha_fatura.display_type == 'product':
        ...
        for tax in taxes:
            if tax.amount not in tax_amount_unique:
                ...
                if tax.amount_type == 'percent':
                    et.SubElement(tax_table_entry, 'TaxPercentage').text = str(abs(tax.amount))
                else:
    ...
    def _write_invoice(self, invoice, eparent, versao):
        if invoice.journal_id.saft_inv_type and invoice.internal_number:
            ...
            et.SubElement(eparent, u"InvoiceNo").text = (
                        prefixo_numero + ' ' + invoice.internal_number)
        else:
            et.SubElement(eparent, u"InvoiceNo").text = ' '
        ...
        if invoice.move_type == 'out_refund':
            ...
            et.SubElement(eparent,
                          u"InvoiceType").text = invoice.journal_id.saft_inv_type
        ...
        for payments in pagamentoscliente:
            ...
            for rec in payments.move_id._get_reconciled_invoices_partials():
                ...
                if inv.move_type != 'entry':
                    ...
                    # 4.4.4.15.1
                    et.SubElement(document_totals, 'TaxPayable').text = str(TaxPayable)
                    # 4.4.4.15.2
                    et.SubElement(document_totals, 'NetTotal').text = str(round(abs(payments.amount - TaxPayable), 2))
                    # 4.4.4.15.3
        ...
        if True:
            ...
            for picking in stock_picking:
                ...
                for linha_move in picking.move_lines:
                    ...
                    # 4.2.3.20.5
                    et.SubElement(line, 'Quantity').text = str(int(linha_move.product_qty))
                    # 4.2.3.20.6
                    et.SubElement(line, 'UnitOfMeasure').text = linha_move.product_uom.name
                    # 4.2.3.20.7
                    et.SubElement(line, 'UnitPrice').text = str(int(round(linha_move.product_id.list_price, 2)))
                    # 4.2.3.20.8
            ...
            for sale in sale_order:
                ...
                for sale_line in sale.order_line:
                    if sale_line.product_id:
                        ...
                        # 4.3.4.14.5 (Quantity)*
                        et.SubElement(eline, u"Quantity").text = str(int(sale_line.product_uom_qty))
                        ...
                        #     et.SubElement(eline, u"Discount").text = str(sale_line.discount)
                        ...
                        if sale_line.price_unit:
                            et.SubElement(eline, u"UnitPrice").text = str(int(round(sale_line.price_unit - (sale_line.price_unit *
                                                                     (sale_line.discount / 100)) , 2)))
                        else:
                        ...
                        for tax in sale_line.tax_id:
                            ...
                            if tax.saft_tax_type == 'IVA' and tax.amount == 0.0:
                                if not tax.exemption_reason or len(tax.exemption_reason.name) < 6:
                                    raise ValidationError(_('Missing configuration for exemption reason '
                                                            '(must be at least 6 chars long).'))
                                else:
                                    et.SubElement(eline,
                                                  "TaxExemptionReason").text = str(
                                        tax.exemption_reason.name)
                                    et.SubElement(eline, "TaxExemptionCode").text = str(tax.exemption_reason.code)
                    ...
                    if final_date is False or invoice.date <= final_date:
                        ...
                        for invoice_line in invoice.invoice_line_ids:
                            if invoice_line.display_type == 'product':
                                ...
                                # 4.1.4.14.5  Quantity
                                et.SubElement(eline, u"Quantity").text = str(int(invoice_line.quantity))
                                ...
                                # if invoice_line.discount:  # Discount (optional)
                                #     et.SubElement(eline, u"Discount").text = str(invoice_line.discount)
                                ...
                                if invoice_line.price_unit:
                                    et.SubElement(eline, u"UnitPrice").text = str(int(round(invoice_line.price_unit-(invoice_line.price_unit*
                                ...
                                if (invoice.invoice_origin or invoice.name) and invoice.move_type == 'out_refund':
                                    ...
                                    if invoice.invoice_origin:
                                        et.SubElement(ereferences, u"Reference").text = _(invoice.journal_id.saft_inv_type
                                                                                          + ' ' + invoice.invoice_origin[:60])
                                        ...
                                        if cont == 1:
                                            ...
                                            if imposto.saft_tax_type == 'IVA' and imposto.amount == 0.0:
                                                if not imposto.exemption_reason or len(imposto.exemption_reason.name) < 6:
                                                    ...
                                                    et.SubElement(eline, u"TaxExemptionReason").text = \
                                                        _(imposto.exemption_reason.name)
                                                    if versao not in ["1.03_01"]:
                                                        TaxExemptionCode = imposto.exemption_reason.code
