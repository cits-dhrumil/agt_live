...
class AccountMove(models.Model):
        ...
        for invoice in self:
            ...
            for invoice_line in invoice.invoice_line_ids:
                ...
                for line in invoice.invoice_line_ids:
                    ...
                    taxes = \
                        line.tax_ids.compute_all(price_reduce, quantity=line.quantity,
                ...
                if invoice_line.display_type and invoice_line.display_type == 'product':
                    ...
                    iva_tax_found = any(
                        tax.saft_tax_type == 'IVA' for tax in invoice_line.tax_ids)
                    if not iva_tax_found:
                        raise ValidationError(
                            "At least one tax must have saft_tax_type == IVA.")
    ...
    def verificar_diarios(self):
        for invoice in self:
            ...
            if invoice.journal_id.saft_inv_type in ['NC',
                                                    'ND'] and not invoice.invoice_origin:
                raise ValidationError(
    ...
    def verificar_cliente_data(self):
        for invoice in self:
            if invoice.move_type in ('out_invoice', 'out_refund'):
                ...
                if str(invoice.invoice_date) >= (
                        datetime(datetime.now(tz_pt).year, datetime.now(tz_pt).month, 1) +
                        relativedelta(months=1)).strftime('%Y-%m-%d'):
                    raise UserError(
                        'O mês da fatura tem de ser o mês corrente ou inferior.')

                if invoice.journal_id.allow_date and invoice.invoice_date.year < datetime.now(
                        tz_pt).year:
                    raise UserError(
                        _('Não pode validar para anos anteriores ao corrente.'))
            ...
            if invoice.currency_id and invoice.currency_id != invoice.company_id.currency_id:
                ...
                if rate and rate.rate and rate.name:
                    if invoice.state == 'draft':
                        ...
                        invoice.currency_tax_info = 'Taxa de câmbio de ' + str(
                            rate.rate) + ' do dia ' + str(data) + '.'
                if invoice.state != 'draft':
    ...
    def verificar_dados_fatura(self):
        for invoice in self:
            if invoice.move_type == 'out_refund' and (
                    not invoice.reason_cancel or not invoice.invoice_origin):
                raise ValidationError(_('Incompleto\n'
                                        'Os campos "Origem" e "Motivo de Anulação" são de preenchimento obrigatório.'))

            if invoice.partner_id and self.move_type in ['in_invoice',
                                                         'in_refund'] and invoice.ref:
                self.env.cr.execute("""
                            SELECT ref
                            FROM account_move
                            WHERE state not in ('draft','cancel') and
                                  move_type=%s and internal_number!='/'
                                  and ref=%s and partner_id=%s and id != %s """,
                                    (invoice.move_type, invoice.ref,
                ...
                if check_internal_number:
                    raise ValidationError('Duplicado !\n Ja existe uma fatura (' + str(
                        check_internal_number[
                            0]) + ') para esse fornecedor com essa referencia!')
            ...
            if invoice.name.count("/") != 1 and invoice.move_type in (
                    'out_invoice', 'out_refund'):
                raise ValidationError('Config\n O numero tem de ter apenas uma barra.')
            if not invoice.journal_id.refund_sequence and invoice.move_type in (
        ...
        for invoice in self:
            ...
            if invoice.company_id.currency_id and invoice.company_id.currency_id != invoice.currency_id:
                ...
                currency_rate = self.env['res.currency.rate'].search([
                    ...
                    ('name', '<=', str(date_fatura)[:10] + ' 23:59:59')],
                    order='name DESC', limit=1)
                if currency_rate:
                    if round(currency_rate.rate, 5) != round(invoice.cambio,
                                                             5) and invoice.move_type not in (
                            'out_refund', 'in_refund'):
                        raise ValidationError(
    ...
    def create_hash(self):
        ...
        for invoice in self:
            ...
            number = ((
                              invoice.journal_id.saft_inv_type + ' ') if invoice.move_type not in [
            ...
            values = creating_hash.hash(invoice, invoice.journal_id.manual, datadocumento,
                                        datasistema, number, numHash, antigoHash,
                                        totalbruto)
    ...
    def action_post(self):
        ...
        for invoice in self:
            if invoice.journal_id.saft_inv_type == 'FS' and invoice.amount_total >= 1000:
                raise UserError(
                    _('O total da fatura não pode ser igual ou superior a 1000, sendo o díário '
            ...
            if invoice.journal_id.type == 'sale' and (not invoice.journal_id.manual or \
                ...
                raise UserError(
                    _('Não pode publicar movimentos de vendas que nao estejam associados a faturas'))
    ...
    def _post(self, soft=True):
        ...
        for invoice in self:
            if invoice.move_type in (
                    ...
                    'in_refund') and not invoice.hash:
                invoice.certify()
        return post

    def action_cancel(self):
        for invoice in self:
            if not invoice.reason_cancel and invoice.move_type not in ['in_invoice',
                                                                       'in_refund']:
                raise ValidationError(_('Incompleto\n'
    ...
    def _onchange_modo_pagar_vd(self):
        for invoice in self:
            ...
            if invoice.modo_pagar_vd:
                invoice.payment_method_vd_id = (
                        invoice.modo_pagar_vd.inbound_payment_method_line_ids and
    ...
    def _amount_by_group(self):
        for order in self:
            ...
            for line in order.invoice_line_ids:
                price_reduce = line.price_unit * (1.0 - line.discount / 100.0)
                taxes = line.tax_ids.compute_all(price_reduce, quantity=line.quantity,
                                                 product=line.product_id,
            ...
            order.amount_by_group = [(
                ...
                l[1]['saft_tax_type'], l[1]['tax_amount']
            ) for l in res]
            ...
            if not invoice.company_id.pais_certificacao or invoice.company_id.pais_certificacao.code == 'PT':
                ...
                if invoice.move_type in ('out_invoice', 'out_refund'):
                    ...
                    if not invoice.atcud and needs_atcud == 'True':
                        ...
                        codigo_validacao_serie = wizard_alert_atcud._get_codigo_validacao_serie(
                            sequence_id_atcud,
                        ...
                        if not codigo_validacao_serie:
                            if self.env.company.series_at_user and self.env.company.series_at_password:
                                ...
                                result = self.env[
                                    'res.config.settings'].register_automatically(
                                    sequences=sequence_id_atcud)
                                self.env['series.wizard.registration.result'].search(
                                    [('id', '=', result['res_id'])]).evaluate_summary()
                                new_code = wizard_alert_atcud._get_codigo_validacao_serie(
                                    sequence_id_atcud,
                                    invoice.invoice_date)
                                invoice.message_post(
                                    body=_(
                                        'Foi atribuído o código de validação de sequência AT: %s' % new_code))
                            else:
    ...
    reason_cancel = fields.Char(string="Reason for Cancellation", size=64, readonly=True,
                                copy=False)
    ref_pagamento = fields.Char(string="Payment Reference", size=64, readonly=True,
                                copy=False, default='-')
    hash = fields.Char(string="Hash", size=256, readonly=True,
                       help="Unique hash of the invoice.", copy=False)
    hash_date = fields.Datetime(string="Date the hash was generated", copy=False)
    cambio = fields.Float(digits=(2, 6), help="Currency exchange", default=1)
    product_id = fields.Many2one(string="Product", related='invoice_line_ids.product_id')
    internal_number = fields.Char(string="Invoice Number", copy=False)
    currency_tax_info = fields.Text(compute='_get_currency_tax_info', readonly=True,
                                    string="Exchange",
                                    help="Value and date of the exchange rate defined for the date and currency of the invoice")
    ...
    system_entry_date = fields.Datetime(string="Confirmation date", copy=False)
    write_date = fields.Datetime(string="Change date", copy=False)
    modo_pagar_vd = fields.Many2one('account.journal', string="Payment Method",
                                    default=_get_caixa_defeito, copy=False)
    payment_method_vd_id = fields.Many2one('account.payment.method',
                                           string="Payment Mode", copy=False)
    payment_method_vd_id_domain = fields.Char(string="Domain payment_method_vd_id",
                                              copy=False,
                                              compute='_onchange_modo_pagar_vd',
                                              store=False)
    ref_saft_inv_type = fields.Selection(related='journal_id.saft_inv_type',
    ...
    origin_document_status = fields.Selection([
        ...
        ('I', 'Integrated'),
        ('S', 'SourceBilling')],
        string="Original Document Status", copy=False)
    journal_id_is_integrado = fields.Boolean(default=False)
    apresentou_decl_recapitulativa = fields.Boolean(
        string="Presented Recapitulative Declaration", default=False)
    prazo = fields.Char(string="Term", default='2')

    def _compute_qr_code_image(self):
        for invoice in self:
            invoice.qr_code_at_img = self.env['alert.atcud']._compute_qr_code_image(
                invoice.qr_code_at)
    ...
    def _get_qr_code_generation(self):
        for invoice in self:
            ...
            if invoice.hash:
                ...
                quatro_caratecters_hash = _(invoice.hash[0:1]) + _(
                    invoice.hash[10:11]) + _(invoice.hash[20:21]) + \
                                          _(invoice.hash[30:31])
                n_certificado = '1456'
                outras_infos = ''

                invoice.qr_code_at = qr_code_generation.qr_code_at(nif_empresa,
                                                                   nif_cliente,
                                                                   pais_cliente,
                                                                   tipo_documento,
                                                                   doc_state, doc_date,
                                                                   ...
                                                                   espaco_fiscal, round(
                        valor_base_isento, 2),
                                                                   round(valor_base_red,
                                                                         2),
                                                                   round(valor_iva_red,
                                                                         2),
                                                                   round(valor_base_int,
                                                                         2),
                                                                   round(valor_iva_int,
                                                                         2),
                                                                   round(
            ...
            if invoice.move_type in ('out_invoice', 'out_refund'):
                if invoice.state == 'cancel' and invoice.name:
                    if not invoice.hash or not invoice.invoice_date or not invoice.internal_number:
                        raise ValidationError(
                            _('Faltam dados de faturação, como data, número ou hash.'))
        return True
    ...
    def write(self, vals):
        for invoice in self:
            if 'state' in vals and vals['state'] == 'cancel':
                if invoice.invoice_date:
                    if (invoice.invoice_date.year < datetime.now().year) or (
                        ...
                        if not invoice.journal_id.integrado and invoice.move_type in [
                            'out_invoice', 'out_refund']:
                            if not invoice.company_id.pais_certificacao or (
                                    invoice.company_id.pais_certificacao and
            ...
            if 'state' in vals:
                if invoice.state != 'draft' and vals[
                    'state'] == 'draft' and invoice.move_type in ['out_invoice',
                                                                  'out_refund']:
    ...
    def _compute_atcud(self):
        for invoice in self:
            ...
            if not invoice.company_id.pais_certificacao or invoice.company_id.pais_certificacao.code == 'PT':
                needs_atcud = self.env['ir.config_parameter'].sudo().get_param(
                    'needs_atcud')
                if invoice.hash and needs_atcud == 'True' and invoice.move_type in (
                        'out_invoice', 'out_refund'):
    ...
    def _get_sequence(self):
        ...
        if self.move_type in (
                'entry', 'out_invoice', 'in_invoice', 'out_receipt', 'in_receipt') \
